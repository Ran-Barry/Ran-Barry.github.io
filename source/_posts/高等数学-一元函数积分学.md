...
![photo](高等数学-一元函数积分学/基本积分公式.png)
(17) $\int\frac{dx}{\sqrt{x^2-a^2}}=\ln|x+\sqrt{x^2-a^2}|+C$
(18) $\int\frac{dx}{x^2-1}=\frac{1}{2}\ln|\frac{x-1}{x+1}|+C$
(19) $\int\tan^2xdx=\sec^2x-1+C$
(20) $\int\cot xdx=\ln|\sin x|+C$
(21) $\int\tan xdx=-\ln|cos x|+C$
(22) $\int\arcsin xdx=x\arcsin x+\sqrt{1-x^2}$

### 三种主要积分法

#### 第一类换元法(凑微分)

若$\int f(u)du=F(u)+C, and\varphi(x)$可导，则$\int f(\varphi(x))\varphi'(x)dx=\int f(\varphi(x))d\varphi(x)=F(\varphi(x))+C$
...
(2) $\varphi(t)$在$[\alpha,\beta](or[\beta,\alpha])$上具有连续导数，且其值域$R_\varphi=[a,b]$，则$\int_a^bf(x)dx=\int_\alpha^\beta f(\varphi(t))\varphi'(t)dt$
...
**变上限求导的三个类型**
(1) $(\int_{\varphi(x)}^{\psi(x)}f(t)dt)'=\int_{\varphi(x)}^{\psi(x)}\frac{\vartheta f(x,t)}{\vartheta x}dt+f(x,\psi(x))\psi'(x)-f(x,\varphi(x))\varphi'(x)$
...
**积分中值定理**
(1) 若$f(x)$在[a,b]上连续，则$\int_a^bf(x)dx=f(\xi)\int_a^b(b-a),a<\xi<b$
