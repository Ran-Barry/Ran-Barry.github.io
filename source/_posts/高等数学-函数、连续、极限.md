...
常用等价无穷小
当$x\rightarrow0$时
(1) $x\sim\sin x\sim\tan x\sim\arcsin x\sim\arctan x\sim\ln(1+x)\sim e^e-1,(1+x)^a-1\sim ax,1-\cos x\sim\frac{1}{2}x^2,a^x-1\sim x\ln a$
(2) $x-\sin x\sim\frac{x^3}{6},\arcsin x-x\sim\frac{x^3}{6},x-\ln(1+x)\sim\frac{x^2}{2},\tan x-x\sim\frac{x^3}{3},x-\arctan x\sim\frac{x^3}{3}$
